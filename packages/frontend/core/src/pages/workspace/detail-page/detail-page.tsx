  ...
  const onLoad = useCallback(
    (bsPage: BlockSuiteDoc, editor: AffineEditorContainer) => {
      ...
      pageService.getEditorMode = (pageId: string) =>
        docRecordList.record$(pageId).value?.mode$.value ?? 'page';
      pageService.getDocUpdatedAt = (pageId: string) => {
        const linkedPage = docRecordList.record$(pageId).value;
...
export const DetailPage = ({ pageId }: { pageId: string }): ReactElement => {
  ...
  const docRecordList = docsService.docRecordList;
  const docListReady = useLiveData(docRecordList.isReady$);
  const docRecord = docRecordList.record$(pageId).value;
